...
public class StudentUtils {
    ...
    {
        return list.stream()
                .mapToDouble(Student::average) // Sử dụng mapToDouble để chuyển đổi thành DoubleStream
                .average() // Tính trung bình của DoubleStream
                .orElse(0.0); // Giá trị mặc định nếu danh sách trống
    }
    ...
    {
        return list.stream()
                .filter(student -> student.isGoodStudent()) // Sử dụng filter để chọn các học sinh giỏi
                .map(Student::toString) // Sử dụng map để chuyển đổi từ Student sang tên
